...
├── .installer-dumps/               # gitignored — installer tarballs and extracted dirs
...
├── scripts/
│   ├── build_inventory.py          # shared generator (--version 2.5|2.6|2.7|all)
│   └── build-inventory-{XY}.py     # optional thin wrappers per version
...
2. Run from `config-examples/`:
   ...
   ```bash
   python3 scripts/build_inventory.py              # 2.5, 2.6, and 2.7
...
Installer extracted at: config-examples/.installer-dumps/{X.Y}/ansible-automation-platform-containerized-setup-{X.Y}-*/
...
- Update scripts/build_inventory.py and regenerate
