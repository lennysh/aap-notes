#!/usr/bin/env python3
"""Generate annotated containerized installer inventory files for AAP 2.5, 2.6, and 2.7."""
...
class VersionConfig:
    ...
    metrics_in_starter: bool = False
    include_eda_redis: bool = True

    @property
    def out_dir(self) -> Path:
        return REPO_ROOT / self.aap_dir / "containerized"

    @property
    def doc_base(self) -> str:
        return (
            f"https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/"
            f"{self.version}/html/containerized_installation"
        )

    def doc_appendix(self, anchor: str = "") -> str:
        url = f"{self.doc_base}/appendix-inventory-files-vars"
...
VERSIONS: dict[str, VersionConfig] = {
    "2.5": VersionConfig(
        version="2.5",
        aap_dir="AAP25",
        registry_ns="ansible-automation-platform-25",
        include_mcp=False,
    ...
    "2.6": VersionConfig(
        version="2.6",
        aap_dir="AAP26",
        registry_ns="ansible-automation-platform-26",
    ),
    "2.7": VersionConfig(
        version="2.7",
        aap_dir="AAP27",
        registry_ns="ansible-automation-platform-27",
        metrics_in_starter=True,
        include_eda_redis=False,
    ),
}
...
CATALOG: list[tuple[str, str, str | None]] = [
    *section("Common / installer-wide"),
    ...
    ("registry_ns_aap", "Registry namespace for AAP component images", "__REGISTRY_NS_AAP__"),
...
EDA_VARS_27_EXTRA = [
    ("eda_event_stream_pg_username", "EDA event stream PostgreSQL username", "eda_event_stream"),
    ("eda_event_stream_pg_password", "EDA event stream PostgreSQL password (auto-generated if unset)", "<set your own>"),
    ("eda_event_stream_pg_cert_auth", "Use certificate authentication for EDA event stream PostgreSQL", "false"),
    ("eda_event_stream_pg_tls_cert", "Client TLS certificate for EDA event stream PostgreSQL", "/path/to/eda-event-stream-db.crt"),
    ("eda_event_stream_pg_tls_key", "Client TLS key for EDA event stream PostgreSQL", "/path/to/eda-event-stream-db.key"),
    ("eda_event_stream_pg_sslmode", "EDA event stream PostgreSQL sslmode", "prefer"),
    ("eda_event_persistence_deploy_db", "Deploy managed PostgreSQL database for EDA event persistence", "false"),
    ("eda_event_persistence_pg_database", "EDA event persistence PostgreSQL database name", "eda_event_persistence"),
    ("eda_event_persistence_pg_username", "EDA event persistence PostgreSQL username", "eda_event_persistence"),
    ("eda_event_persistence_pg_password", "EDA event persistence PostgreSQL password (auto-generated if unset)", "<set your own>"),
    ("eda_event_persistence_pg_cert_auth", "Use certificate authentication for EDA event persistence PostgreSQL", "false"),
    ("eda_event_persistence_pg_tls_cert", "Client TLS certificate for EDA event persistence PostgreSQL", "/path/to/eda-event-persistence-db.crt"),
    ("eda_event_persistence_pg_tls_key", "Client TLS key for EDA event persistence PostgreSQL", "/path/to/eda-event-persistence-db.key"),
    ("eda_event_persistence_pg_sslmode", "EDA event persistence PostgreSQL sslmode", "prefer"),
]
...
LIGHTSPEED_VARS_27_EXTRA = [
    ("lightspeed_idle_timeout", "Computed: max(client_request_timeout, 60) seconds for Lightspeed idle timeout", "60"),
    ("lightspeed_request_timeout", "Computed: max(lightspeed_streaming_timeout, client_request_timeout) for Lightspeed requests", "3600"),
    ("lightspeed_streaming_timeout", "Timeout in seconds for streaming Lightspeed chat operations", "3600"),
    ("lightspeed_chatbot_byok_image", "BYOK container image with FAISS vector DB at /rag/vector_db/", "quay.io/example/byok-rag-content:latest"),
    ("lightspeed_chatbot_byok_score_multiplier", "BYOK content score multiplier in Lightspeed search results", "1.2"),
]
...
METRICS_VARS = [
    *section("Automation Metrics Service (AAP 2.6+)"),
    ...
    ("automationmetrics_controller_read_pg_host", "Controller PostgreSQL host for read-only metrics access", "externaldb.example.org"),
    ("automationmetrics_controller_read_pg_cert_auth", "Use certificate authentication for read-only Controller PostgreSQL", "false"),
    ("automationmetrics_controller_read_pg_port", "Controller PostgreSQL port for read-only metrics access", "5432"),
    ("automationmetrics_controller_read_pg_sslmode", "Read-only Controller PostgreSQL sslmode", "prefer"),
    ("automationmetrics_controller_read_pg_tls_cert", "Client TLS certificate for read-only Controller PostgreSQL", "/path/to/controller-read-db.crt"),
    ("automationmetrics_controller_read_pg_tls_key", "Client TLS key for read-only Controller PostgreSQL", "/path/to/controller-read-db.key"),
    ("automationmetrics_controller_read_pg_tls_remote", "Read-only Controller DB TLS files are on the Metrics host", "false"),
    ...
    ("automationmetrics_pg_database", "Metrics Service PostgreSQL database name", "metrics_service"),
    ("automationmetrics_pg_host", "Metrics Service PostgreSQL host", "externaldb.example.org"),
    ("automationmetrics_pg_password", "Metrics Service PostgreSQL password", "<set your own>"),
...
METRICS_VARS_27_EXTRA = [
    ("automationmetrics_skip_install", "Skip Metrics Service when [automationcontroller] is present", "false"),
]

OPTIONAL_IMAGE_VARS = {"automationmetrics_image", "mcp_image", "mcp_tools_image"}

BASE_ENTERPRISE_ENABLED = {
...
def localize_entries(entries: list[tuple[str, str, str | None]], cfg: VersionConfig) -> list[tuple[str, str, str | None]]:
    localized: list[tuple[str, str, str | None]] = []
    for name, comment, example in entries:
        if isinstance(comment, str):
            comment = re.sub(r"/2\.\d/", f"/{cfg.version}/", comment)
        if example == "__REGISTRY_NS_AAP__":
            example = cfg.registry_ns
        localized.append((name, comment, example))
    return localized


def catalog_for(cfg: VersionConfig) -> list[tuple[str, str, str | None]]:
    entries = localize_entries(CATALOG, cfg)
    if not cfg.include_metrics:
        entries = [entry for entry in entries if entry[0] != "automationmetrics_image"]
    if not cfg.include_mcp:
        entries = [entry for entry in entries if entry[0] not in {"mcp_image", "mcp_tools_image"}]
    return entries


def eda_vars_for(cfg: VersionConfig) -> list[tuple[str, str, str | None]]:
    entries = list(EDA_VARS)
    if cfg.include_eda_redis:
        insert_at = next(i for i, entry in enumerate(entries) if entry[0] == "eda_pg_username") + 1
        entries[insert_at:insert_at] = EDA_REDIS_VARS
    if cfg.version == "2.7":
        entries.extend(EDA_VARS_27_EXTRA)
    return localize_entries(entries, cfg)


def lightspeed_vars_for(cfg: VersionConfig) -> list[tuple[str, str, str | None]]:
    entries = list(LIGHTSPEED_VARS)
    if cfg.version == "2.7":
        entries.extend(LIGHTSPEED_VARS_27_EXTRA)
    return localize_entries(entries, cfg)


def all_optional_for(cfg: VersionConfig) -> list[tuple[str, str, str | None]]:
    parts = [
        catalog_for(cfg),
        localize_entries(GATEWAY_VARS, cfg),
        localize_entries(CONTROLLER_VARS, cfg),
        localize_entries(HUB_VARS, cfg),
        eda_vars_for(cfg),
        lightspeed_vars_for(cfg),
    ]
    if cfg.include_mcp:
        parts.append(localize_entries(MCP_VARS, cfg))
    if cfg.include_metrics:
        metrics = list(METRICS_VARS)
        if cfg.version == "2.7":
            metrics.extend(METRICS_VARS_27_EXTRA)
        parts.append(localize_entries(metrics, cfg))
...
def enterprise_enabled_for(cfg: VersionConfig) -> set[str]:
    enabled = set(BASE_ENTERPRISE_ENABLED)
    if cfg.metrics_in_starter:
        enabled |= METRICS_ENTERPRISE_ENABLED
    return enabled


def growth_enabled_for(cfg: VersionConfig) -> set[str]:
    enabled = set(BASE_GROWTH_ENABLED)
    if cfg.metrics_in_starter:
        enabled |= METRICS_GROWTH_ENABLED
    return enabled


def enterprise_enabled_values_for(cfg: VersionConfig) -> dict[str, str]:
    values = {
...
def growth_enabled_values_for(cfg: VersionConfig) -> dict[str, str]:
    values = {
...
def lightspeed_comment_block(cfg: VersionConfig, growth: bool = False) -> str:
    pg_host = "aap.example.org" if growth else "externaldb.example.org"
    return f"""# AAP Lightspeed
# {cfg.doc_appendix('#lightspeed-variables')}
# -----------------------------------------------------
# lightspeed_admin_password=<set your own>
# lightspeed_pg_host={pg_host}
...
def enterprise_hosts_for(cfg: VersionConfig) -> list[str]:
    hosts = [
    ...
    if cfg.include_metrics:
        if cfg.metrics_in_starter:
            hosts.extend(
                [
                    "# Automation Metrics Service (required when [automationcontroller] is deployed)",
                    "# -----------------------------------------------------",
                    "[automationmetrics]",
...
def growth_hosts_for(cfg: VersionConfig) -> list[str]:
    hosts = [
        "# This section is for your AAP Gateway host(s)",
        "# -----------------------------------------------------",
        "[automationgateway]",
...
def enterprise_header_for(cfg: VersionConfig) -> list[str]:
    return [
        ...
        f"# {cfg.doc_base}",
        "",
        f"# ENABLED lines below match the upstream AAP {cfg.version} starter inventory.",
        "# All other supported variables are listed commented out for reference.",
        "",
    ]


def growth_header_for(cfg: VersionConfig) -> list[str]:
    return [
        ...
        f"# https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/{cfg.version}/html/tested_deployment_models/container-topologies",
        "#",
        "# Please consult the docs if you're unsure what to add",
        "# For all optional variables please consult the included README.md",
        ...
        f"# {cfg.doc_base}",
        "",
        f"# ENABLED lines below match the upstream AAP {cfg.version} inventory-growth starter.",
        "# All other supported variables are listed commented out for reference.",
        "",
    ]
...
def build_vars_example(cfg: VersionConfig, entries: list[tuple[str, str, str | None]]) -> str:
    """Build vars-example.yml with every YAML-only variable commented out."""
    lines = [
        "---",
        f"# AAP {cfg.version} optional variables — YAML-only (lists and dictionaries)",
        "#",
        ...
        "#",
        f"# Regenerate from catalog: python3 scripts/build_inventory.py --version {cfg.version}",
        "",
    ]
...
def build_enabled_section(cfg: VersionConfig, enabled: set[str], values: dict[str, str]) -> list[str]:
    order = [
        ("ansible_connection", "Ansible", None),
        ("Common variables", "Common variables", cfg.doc_appendix("#general-variables")),
        ("gateway", "AAP Gateway", cfg.doc_appendix("#platform-gateway-variables")),
        ("controller", "AAP Controller", cfg.doc_appendix("#controller-variables")),
        ("hub", "AAP Automation Hub", cfg.doc_appendix("#hub-variables")),
        ("eda", "AAP EDA Controller", cfg.doc_appendix("#event-driven-ansible-variables")),
    ]
    if cfg.metrics_in_starter:
        order.append(
            (
                ...
                cfg.doc_appendix("#automation-metrics-service-variables"),
            )
        )
    ...
    groups = {
        ...
        "automationmetrics": METRICS_ENTERPRISE_ENABLED | METRICS_GROWTH_ENABLED,
    }
...
def build_file(
    ...
    enabled: set[str],
    values: dict[str, str],
    lightspeed_block: str,
    optional_entries: list[tuple[str, str, str | None]],
    topology_skip_extra: set[str],
) -> str:
    skip = enabled | LIGHTSPEED_STARTER | topology_skip_extra
    optional = render_optional_vars(optional_entries, skip)

    parts = header + host_sections + build_enabled_section(cfg, enabled, values)
...
def generate_version(cfg: VersionConfig) -> None:
    out_dir = cfg.out_dir
    out_dir.mkdir(parents=True, exist_ok=True)
    optional_entries = all_optional_for(cfg)

    enterprise = build_file(
        cfg,
        enterprise_header_for(cfg),
        enterprise_hosts_for(cfg),
        enterprise_enabled_for(cfg),
        enterprise_enabled_values_for(cfg),
        lightspeed_comment_block(cfg),
        optional_entries,
        set(),
    )
    growth = build_file(
        cfg,
        growth_header_for(cfg),
        growth_hosts_for(cfg),
        growth_enabled_for(cfg),
        growth_enabled_values_for(cfg),
        lightspeed_comment_block(cfg, growth=True),
        optional_entries,
        set(),
    )
    ...
    (out_dir / "vars-example.yml").write_text(build_vars_example(cfg, optional_entries), encoding="utf-8")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate annotated containerized installer inventories.")
    parser.add_argument(
        "--version",
