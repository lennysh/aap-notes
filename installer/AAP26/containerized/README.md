...
For OpenShift operator manifests, see [AAP 2.6 on OpenShift](../../openshift/AAP26/README.md). For RPM deployment, see [AAP 2.6 RPM inventories](../rpm/README.md). For maintenance when adding versions or refreshing after installer updates, see [CONTAINERIZED_INVENTORY_CONTEXT.md](../../maintainer/CONTAINERIZED_INVENTORY_CONTEXT.md).
...
Variable definitions live in [`scripts/build_inventory.py`](../../scripts/build_inventory.py). When Red Hat ships a new 2.6 installer refresh, diff the upstream README and role defaults against that catalog, then regenerate. See [CONTAINERIZED_INVENTORY_CONTEXT.md](../../maintainer/CONTAINERIZED_INVENTORY_CONTEXT.md) for the full workflow.
